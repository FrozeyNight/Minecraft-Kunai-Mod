...
public class KunaiItem extends SwordItem {
    ...
    public static void gravityPullToKunai(Player player, Level world){
        ...
        if (player instanceof ServerPlayer) {
            ...
            List<LivingEntity> nearbyEntities = world.getEntitiesOfClass(LivingEntity.class,
                    new AABB(daggerPos.x() - 5.0, daggerPos.y() - 5.0, daggerPos.z() - 5.0,
                            daggerPos.x() + 5.0, daggerPos.y() + 5.0, daggerPos.z() + 5.0));
            ...
            for (LivingEntity entity : nearbyEntities) {
                ...
                double strength = 1;  // Adjust this value to control the strength of the gravity pull
                entity.setDeltaMovement(direction.x * strength, direction.y * strength, direction.z * strength);

                entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 40, 255, false, false));
            }
    ...
    public static void BlindingKunai(Player player, Level world){
        ThrownKunai lastThrownKunai = findLastThrownKunai(player);
        ...
        if (player instanceof ServerPlayer) {
            ...
            for (LivingEntity entity : nearbyEntities) {
                entity.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 200, 255));
            }
            ...
            world.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.WARDEN_NEARBY_CLOSER, SoundSource.PLAYERS, 2f , 1f);
